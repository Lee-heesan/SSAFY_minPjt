...
document.querySelector("#btn_apt_search").addEventListener("click", async () => {
  ...
  for (let item of items) {
    ...
    const dealAmount = item.getElementsByTagName("dealAmount")[0].textContent;
    ...
    const excluUseAr = item.getElementsByTagName("excluUseAr")[0].textContent;
    infos.push({
      address: address,
      utmk: await getCoords(address),
      label: aptNm,
